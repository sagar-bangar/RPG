...
public class CharacterMoveState : CharacterBaseState
{
    ...
    private Vector3 MoveDirection()
    {
        if (_ctx.OnGround())
        {
            Vector3 normal = (_ctx._hit.normal);
            Vector3 z = Vector3.Cross(normal, _ctx.transform.right).normalized;
            Vector3 x = Vector3.Cross(normal, _ctx.transform.forward).normalized;
            ...
            if (_ctx._hit.normal.y > 0.5) // 60deg wit transfor.up cos60=0.5
            {
                _ctx._moveDir.y = _ctx._hit.normal.y * 0.00001f; // Dot for lesser roataion than hit.normal
            }
            else // Hit.normal=mgcos0 
            {
                _ctx._moveDir.y = _ctx._hit.normal.y; // Hit.normal=1 which is 90deg with transform.up cos90=1
            }
